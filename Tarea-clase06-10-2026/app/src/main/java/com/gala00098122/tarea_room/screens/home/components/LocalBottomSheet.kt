...
fun LocalBottomSheet(
  ...
  var votes by rememberSaveable { mutableIntStateOf(0) }
  
  val isValid = name.isNotBlank() && imageUrl.isNotBlank() && votes > 0
  ...
  ModalBottomSheet(
    ...
    Column(
      ...
      OutlinedTextField(
        ...
        onValueChange = { votes = it.toIntOrNull() ?: 0 },
        label = { Text("Votos") },
        modifier = Modifier.fillMaxWidth(),
        singleLine = true
      )
      ...
      Row(
        ...
        Button(
          onClick = {
            if (isValid) {
              onSave(name.trim(), imageUrl.trim(), votes)
              onDismiss()
            }
          },
          enabled = isValid
